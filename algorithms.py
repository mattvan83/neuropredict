...
def get_DecisionTreeClassifier(train_class_sizes,
                               reduced_dim=None,
...
def get_ExtraTreesClassifier(train_class_sizes,
                             reduced_dim=None,
                             grid_search_level=cfg.GRIDSEARCH_LEVEL_DEFAULT,
                             random_state=42):
    """
...
def get_LogisticRegression(train_class_sizes,
                           reduced_dim=None,
                           grid_search_level=cfg.GRIDSEARCH_LEVEL_DEFAULT,
                           random_state=42):
    """
    Returns the Logistic Regression classifier and its parameter grid.
    ...
    if grid_search_level in ['exhaustive']:
        ...
        range_penalty = np.logspace(-25, 17, 43, base = 2)
        # np.power(10.0, range(-3, 6))
    ...
    ## Use LibLinear
    if reduced_dim < np.sum(train_class_sizes):
        clf = LogisticRegression(penalty='l2', dual=False, random_state=random_state, solver='liblinear')
    else:
        clf = LogisticRegression(penalty='l2', dual=True, random_state=random_state, solver='liblinear')

    return clf, clf_name, param_grid

def get_LinearSVC(train_class_sizes,
                  reduced_dim=None,
                  grid_search_level=cfg.GRIDSEARCH_LEVEL_DEFAULT,
                  random_state=42):
    """
    Returns the Linear SVM classifier and its parameter grid.
    ...
    # ## Use LibLinear
    # if reduced_dim < np.sum(train_class_sizes):
    #     clf = LinearSVC(penalty="l2", loss="squared_hinge", dual=False, random_state=random_state)
    # else:
    #     clf = LinearSVC(penalty="l2", loss="squared_hinge", dual=True, random_state=random_state)
...
def get_svc(train_class_sizes, reduced_dim=None, grid_search_level=cfg.GRIDSEARCH_LEVEL_DEFAULT, random_state=42):
    """
...
def get_RandomForestClassifier(train_class_sizes,
                               reduced_dim=None,
                               grid_search_level=cfg.GRIDSEARCH_LEVEL_DEFAULT,
                               random_state=42):
    """
...
def get_xgboost(train_class_sizes,
                reduced_dim=None,
                grid_search_level=cfg.GRIDSEARCH_LEVEL_DEFAULT,
                random_state=42):
    """
...
def get_classifier(train_class_sizes,
    ...
    clf, clf_name, param_grid = clf_builder(train_class_sizes, reduced_dim, grid_search_level)
...
def get_pipeline(train_class_sizes, feat_sel_size, num_features,
    ...
    estimator, est_name, clf_param_grid = get_classifier(train_class_sizes, classifier_name, reduced_dim, grid_search_level)
